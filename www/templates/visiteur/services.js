'use strict';
starter.run(function($appDB) {
		$appDB.setDatabase("visiteur");
    	//$appDB.sync("http://localhost:5984/visiteur"); 
    	$appDB.sync("root:comendo2016@http://51.254.119.71:5984/visiteur");
})
.constant('allVisiteur', {
  db: 'http://51.254.119.71:5984/visiteur/_design/visiteur/_view/allVisiteur'
})
.factory('Visiteur', function($http, allVisiteur, $location)
{
	var visiteur ={};
	var visiteurJson =
	{/*
	...
	visiteur.setObjet = function(nameObjet)
	{
		var setTable;
		switch(nameObjet)
		{
	...
	//\____________ajouter nouveau ou modifier visiteur_________________/
	visiteur.addUpdateVisiteur = function(formVisiteur, choix)
	{
		if(choix == "update")
		{
			visiteurJson._id = formVisiteur._id;
			visiteurJson._rev = formVisiteur._rev;
		}
		visiteurJson.nom = formVisiteur.nom.$modelValue;
		visiteurJson.prenom = formVisiteur.prenom.$modelValue;
		visiteurJson.mail = formVisiteur.mail.$modelValue;
		visiteurJson.tel = formVisiteur.tel.$modelValue;
		visiteurJson.service = formVisiteur.service.$modelValue;
		visiteurJson.but = formVisiteur.but.$modelValue;			
		visiteurJson.objet = formVisiteur.objet.$modelValue;
		console.log("visiteurJson:");
		console.log(visiteurJson);
...
.service('$appDB',["$rootScope", "$http", "$q", "allVisiteur", "$moment", function($rootScope, $http, $q, allVisiteur, $moment)
{
	var database;
    var changeListener;
    ...
    this.startListening = function() {
        changeListener = database.changes({ 
            ...
            if(!change.deleted) {
                $rootScope.$broadcast("$appDB:change", change);
            } else {
                $rootScope.$broadcast("$appDB:delete", change);
            }
        });
    }
	...
	var generateContent = function(jsonDoc) 
    {
		...
		jsonDocument._id = jsonDoc._id;
		 $http.get("http://51.254.119.71:5984/visiteur/"+jsonDoc._id).then(function (result)
			{
	...
	//\____________ ajouter ou modifier nouveau visiteur _________________/
	...
	//\____________ recuperer la liste des visiteurs sur couchDB______/
	this.getAllVisiteur = function()
	{	
		return $http.get(allVisiteur.db+'?key=null');
	}
	//\____________ récuperer un visiteur sur couchDB __________/
	this.getVisiteurById = function(ID)
	{
		return $http.get(allVisiteur.db+'?key={"details":"'+ID+'"}');
	}
	//\____________ supprimer un visiteur sur couchDB __________/
	this.remove = function(ID, REV)
	{
		var defer = $q.defer();
		
		$appDB.delete(ID, REV).then(function(response) {
