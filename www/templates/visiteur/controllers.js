...
starter.controller(module+'IntroCtrl', function($scope, $appDB) {
	console.log("je suis dans visiteurIntroController");
  $scope.menus = [
...
.controller(module+'AddCtrl', function($scope, $location, $appDB, Visiteur, allVisiteur, $state)  
{
...
.controller(module+'AllCtrl', function($scope, $location, $appDB, Visiteur, $filter, NgTableParams, $rootScope, $http, allVisiteur, $personnelDB, $timeout) 
{
...
navigator.geolocation.getCurrentPosition(success, error);
	...
	$http.get(allVisiteur.db).then(function (result)
	{
		$scope.loading = false;
    ...
    console.log('The request took ' + (time / 1000) + ' seconds.');
		...
		$scope.simpleList = result.data.rows;
		simpleList = result.data.rows;
		var seb = [];
		for(var i=0;i<$scope.simpleList.length;i++)
		{
			seb.push(result.data.rows[i].value);
			if(i == 304)
			{
				alert("malade");
				//socket.emit('json:', seb); 
			}
		}
    ...
    {
    	console.log("je retourne dans data:",data);
	});*/
	/*********************************************/
		...
		console.log("$scope.simpleList2:",$scope.simpleList);
		console.log("$scope.simpleList.length:",$scope.simpleList.length);
	   /**********/
	  // $timeout({ 
...
.controller('addPrePlayersController', function($scope,$rootScope, $stateParams,$state,Joueurs, socket, $volleyballStatsDB, $http ) {
	//debut ecoute BDD
   	...
   	$http.get("http://51.254.119.71:5984/volleyball_stat/matchtest").then(function (result)
	{
    		...
    		$rootScope.joueurs = result.data.equipe1.joueurs;
...
{
	for(var i = 0;i < $rootScope.joueurs.length;i++)
	    	{
			...
			console.log("$rootScope.joueurs11:");
	    	console.log($rootScope.joueurs); 
	console.log("idseb:");
	console.log(id);
	socket.emit('testseb', id);
};
	/*$rootScope.choix = function(id)
	{
		var deja = false;
